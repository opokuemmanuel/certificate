            ...
            <div class="card-body">
                <table class="table table-bordered table-responsive-lg">
                    ...
                    @foreach($pro as $prodd)
                        <tr>
                            <td>{{$prodd->program}}</td>
                            <td>{{$prodd->department}}</td>
                            <td>{{$prodd->school}}</td>
                            <td>
                                <a href="{{url('/EditProgram/'.$prodd->id)}}" class="btn btn-success" style="color: white; margin-top: 5px">Edit</a>
                                <form action="{{ route('deleteProgram') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="university_name" value="{{ Auth::user()->university_name}}">
